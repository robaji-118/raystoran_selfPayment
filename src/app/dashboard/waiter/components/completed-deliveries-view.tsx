        ...
        <div className="flex items-center gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 bg-gray-100 text-black rounded text-[10px] font-bold uppercase tracking-wider border border-purple-200">
                Completed
              </span>
            </div>
        ...
        <div className="border-t border-gray-100 bg-gray-50/50 p-5 animate-in slide-in-from-top-2 duration-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
            ...
            <div className="flex gap-3">
              <div className="flex-1 bg-white p-3 rounded-lg border border-gray-200 flex flex-col items-center justify-center text-center">
                <div className="flex items-center gap-1.5 text-xs text-gray-500 mb-1">
                  <Clock className="w-3.5 h-3.5 text-blue-500" />
                  Delivery
                </div>
                <span className="font-bold text-gray-900">{deliveryTime} min</span>
              </div>
              <div className="flex-1 bg-white p-3 rounded-lg border border-gray-200 flex flex-col items-center justify-center text-center">
                <div className="flex items-center gap-1.5 text-xs text-gray-500 mb-1">
                ...
                <span className="font-bold text-gray-900">{totalTime} min</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 divide-y divide-gray-100">
              {order.items.map((item, index) => (
                <div key={`${item._id}-${index}`} className="p-3 flex justify-between items-start">
                  <div className="flex gap-3">
                    <span className="flex items-center justify-center w-6 h-6 bg-gray-100 rounded text-xs font-bold text-gray-700">
                      {item.quantity}x
                    </span>
                    <div>
                      <p className="text-sm font-medium text-gray-900">{item.menuItemName}</p>
                      {item.notes && (
                        <p className="text-xs text-orange-600 italic mt-0.5">Note: {item.notes}</p>
                      )}
                    </div>
                  </div>
              ...
              <div className="p-3 bg-gray-50 flex justify-between items-center rounded-b-lg">
                ...
                <span className="text-base font-bold text-gray-900">{formatRupiah(order.totalAmount)}</span>
              </div>
            </div>
          </div>
    ...
    <div className="p-6 min-h-screen bg-gray-50/50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-gray-500 text-sm font-medium mb-1">Total Delivered</p>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-gray-500 text-sm font-medium mb-1">Avg Delivery Time</p>
            <p className="text-3xl font-bold text-gray-900">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-gray-500 text-sm font-medium mb-1">Items Delivered</p>
            <p className="text-3xl font-bold text-gray-900">
              {stats.totalItems}
            </p>
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent shadow-sm"
          />
        </div>
