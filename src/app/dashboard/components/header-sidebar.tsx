...
const HeaderSidebar: React.FC<HeaderProps> = ({
  ...
  const getRoleColor = (role: string): string => {
    const colors: Record<string, string> = {
      admin:
        "bg-neutral-100 text-fluid-sm text-black rounded-full items-center justify-center",
      ...
      <div className="px-fluid-6 py-fluid-4">
        <div className="flex items-center justify-between gap-fluid-4">
          ...
          <div className="flex items-center gap-fluid-3">
            ...
            {/* User Info */}
            <div className="hidden sm:flex items-center gap-fluid-3 pl-fluid-3 border-l border-neutral-200">
              {/* Bagian Teks (Nama & Role) */}
              <div className="flex flex-col items-end justify-center gap-1">
                {" "}
                {/* Tambahkan items-end & justify-center */}
                <p className="text-sm font-bold text-neutral-950 leading-none">
                  {userName}
                </p>
                {/* Badge Role */}
                <span className="inline-flex items-center justify-center bg-neutral-100 px-2 py-0.5 rounded-full text-xs font-medium text-black mt-1">
                  {getRoleLabel(userRole)}
                </span>
              </div>

              {/* Avatar */}
              <div className="w-10 h-10 bg-black uppercase rounded-full flex items-center justify-center text-white font-semibold text-lg shrink-0">
                {userName
                  .split(" ")
                  .map((n) => n[0])
                  .join("")
