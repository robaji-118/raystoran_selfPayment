// app/dashboard/customer/steps/step3-drink-selection.tsx
"use client";

import { useState, useEffect } from "react";
import { Plus, Minus, Coffee, Clock, ImageIcon, ChefHat } from "lucide-react";
...
interface Step3DrinkSelectionProps {
  cart: CartItem[];
...
export default function Step3DrinkSelection({
  cart,
  onAddToCart,
  onUpdateQuantity,
}: Step3DrinkSelectionProps) {
  ...
  const fetchMenuItems = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Filter hanya untuk minuman
        const drinkItems = items.filter((item: MenuItem) => {
    ...
    <div className="space-y-6">
      {menuItems.length > 0 ? (
        ...
        <div className="text-center py-24 bg-white rounded-3xl shadow-lg">
          ...
          <p className="text-gray-900 font-bold text-xl mb-3">No Drink Items Available</p>
